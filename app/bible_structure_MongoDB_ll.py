...
def _parse_paragraph(p_tag: Tag) -> Paragraph:
    ...
    Strategy:
      1. Get the raw text of the entire div including '#' delimiters.
      ...
      4. Trailing bible text inside a non-last block carries forward to the
         next chunk.
      5. Verse numbers (digits) embedded in bible text are detected and used
         to split into separate Verse objects.
    """
    # Collect raw text preserving '#' markers from ft spans
    raw_parts: list[str] = []
    ...
    # Split on '#' into [bible_text, note_block, note_block, ...]
    parts      = raw.split("#")
    note_parts = parts[1:]
    chunks: list[Chunk] = []
    ...
    for chunk in chunks:
        # Check if this chunk's text starts with a verse number
