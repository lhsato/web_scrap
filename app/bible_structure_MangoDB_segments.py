...
def _parse_footnote_span(ft_tag: Tag) -> list[Footnote]:
    ...
    A single span contains one or more concatenated notes, e.g.:
      "tn Some translation note. sn Some study note."
      "tn First note.sn Second note."   <- glued directly after punctuation

    The type is always the first 2 characters of each note (tn, sn, tc).
    A new note boundary is detected ONLY after punctuation (. , ; : ! ? ) ] })
    followed by a known prefix + space — this prevents false splits on words
    like "isn\'t" (contains sn) or mid-sentence occurrences of prefix strings.
    """
    raw = re.sub(r"\s+", " ", ft_tag.get_text(" ", strip=True)).strip()

    # Split on: punctuation + optional space + known prefix + space
    segments = re.split(r"(?<=[.;,!?)\]})\s*(tn|sn|tc)\s+", raw)

    results: list[Footnote] = []

    # First segment always opens with the first prefix
    first = segments[0].strip()
    if first:
        m = re.match(r"^(tn|sn|tc)\s+(.*)", first, re.DOTALL)
        if m:
            results.append(Footnote(type=m.group(1), text=m.group(2).strip()))  # type: ignore[arg-type]
        else:
            results.append(Footnote(type="unknown", text=first))

    # Remaining pairs are (captured delimiter, body)
    i = 1
    while i < len(segments) - 1:
        prefix = segments[i].strip()
        body   = segments[i + 1].strip() if i + 1 < len(segments) else ""
        if prefix in KNOWN_PREFIXES and body:
            results.append(Footnote(type=prefix, text=body))  # type: ignore[arg-type]
        i += 2

    return results if results else ([Footnote(type="unknown", text=raw)] if raw else [])
...
def scrape(url: str) -> ChapterPage:
    ...
    content_tags = soup.find_all(
        ...
        # pega todos as classes que contenham CLS_FT
        # class_=lambda c: c and (CLS_FT in c) 
    )
    ...
    for tag in content_tags:
        ...
        if CLS_HEADING in tag_classes:
            ...
            # pega todos os ft dentro do paragrafo
            footnotes = tag.find_all(class_=CLS_FT)
