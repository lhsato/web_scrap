...
def _parse_paragraph(p_tag: Tag) -> Paragraph:
    ...
    for child in p_tag.children:
        ...
        if isinstance(child, Tag):
            ...
            # Detect verse number: any tag whose entire text is a plain integer.
            # Guard with CLS_FT check so footnote spans that happen to start
            # with a number are never misclassified as verse numbers.
            raw_num = child.get_text(strip=True)
            if re.fullmatch(r"\d+", raw_num) and CLS_FT not in classes:
                flush_verse()
            ...
            if CLS_FT in classes:
                ...
                if fns:
                    # Attach footnotes directly to the preceding text chunk
                    text = re.sub(r"\s+", " ", current_chunk_text).strip()
                    if current_verse is not None:
                        current_verse.chunks.append(Chunk(text=text, footnotes=fns))
                    current_chunk_text = ""
                    current_footnotes = []
                continue
...
if __name__ == "__main__":
    ...
    print(f"  .plain_text               → {v1.plain_text[:100000]!r}")
    # print(f"  .all_footnotes[0]         → {v1.all_footnotes[0]}")
    # print(f"  .all_footnotes[0].type    → {v1.all_footnotes[0].type!r}")

    # parei aqui  print(f"  .plain_text               → {v1.plain_text[:100000]!r}") tem so o primeiro versiculo
